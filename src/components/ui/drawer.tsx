...
>(({ className, children, ...props }, ref) => (
  <DrawerPortal>
    ...
    <DrawerPrimitive.Content
      ref={ref}
      className={cn(
        "fixed inset-x-0 z-50 mt-24 flex h-auto flex-col rounded-t-[16px] border bg-background outline-none focus:outline-none focus-visible:outline-none",
        className,
      )}
...
const DrawerHeader = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div className={cn("grid gap-1.5 px-4 pb-0 text-center sm:text-left", className)} {...props} />
);
