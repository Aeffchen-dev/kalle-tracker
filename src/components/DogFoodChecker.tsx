  ...
  return (
    <div className="mb-8" ref={containerRef}>
      <div className="glass-card rounded-lg p-4">
        <div className="flex items-center gap-2 mb-3 border border-black/15 overflow-hidden relative" style={{ borderRadius: '99px', padding: '4px 4px 4px 14px', background: 'rgba(255,255,255,1)' }}>
          ...
          <div className="relative" style={{ width: `${inputWidth}px` }}>
            <input
              ...
              className="w-full min-w-0 bg-transparent text-[13px] text-black outline-none text-center transition-all duration-150"
              style={{ caretColor: 'black' }}
              placeholder=""
            />
            {!query && !isFocused && (
              <span className="absolute inset-0 flex items-center justify-center text-[13px] text-black/25 pointer-events-none">
                {placeholderText}
          ...
          <span className="text-[13px] text-black shrink-0 -ml-1">essen?</span>
          </div>
          <div className="ml-auto">
          <button
            onClick={checkFood}
            disabled={loading || !query.trim()}
            className="shrink-0 rounded-full bg-black border border-black/20 active:scale-95 transition-all flex items-center justify-center w-[36px] h-[36px] cursor-pointer"
          >
            {loading ? (
