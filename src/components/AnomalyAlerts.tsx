      ...
      {anomalies.map((anomaly) => {
        ...
        return (
          ...
          >
            ...
            >
              {anomaly.type === 'connection_error' ? <AlertCircle size={20} className="text-white flex-shrink-0" /> : <span className="text-[20px]">{getEmoji(anomaly.type)}</span>}
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2">
              <span className={`text-[16px] truncate ${anomaly.type === 'connection_error' ? 'text-white' : 'text-black'}`}>
                    {anomaly.title}
                  </span>
                  <span className={`text-[14px] flex-shrink-0 ${anomaly.type === 'connection_error' ? 'text-white/70' : 'text-black/50'}`}>
                    {format(anomaly.timestamp, 'd. MMM', { locale: de })}
                  </span>
                </div>
                <p className={`text-[16px] truncate ${anomaly.type === 'connection_error' ? 'text-white/80' : 'text-black/70'}`}>
