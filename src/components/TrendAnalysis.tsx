...
const GrowthCurveChart = memo(({ events, width }: { events: Event[]; width: number }) => {
  ...
  // Y-axis ticks - starting at 7kg with 5kg steps
  const yTicks = [7, 12, 17, 22, 27, 32];
  const domainMin = 7;
  const domainMax = 35;
  const totalHeight = CHART_HEIGHT + X_AXIS_HEIGHT;
    ...
    <div>
      <div className="relative flex">
        ...
        >
          <svg width={yAxisWidth} height={CHART_HEIGHT}>
            {yTicks.map((tick) => {
              // Match Recharts internal calculation: top margin = 8, plotting area = CHART_HEIGHT - 8
              const plotHeight = CHART_HEIGHT - 8;
