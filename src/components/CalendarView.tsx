              ...
              {filteredEvents.length === 0 && !isBirthdayToday && filteredIcalEvents.filter(evt => !/hat\s+Kalle/i.test(evt.summary || '')).length === 0 && predictionSlots.length === 0 ? (
                ...
                <div className="space-y-2 pb-20">
                  ...
                  {isBirthdayToday && birthdayAge > 0 && (
                    <div className="flex items-center justify-between p-3 bg-white/[0.06] backdrop-blur-[12px] rounded-lg shadow-[0_2px_8px_rgba(0,0,0,0.12)]">
                      ...
                      if (isPipiGroup && group.events.length > 0) {
                        ...
                        return (
                          <div key={`group-${gi}`} className="relative flex w-full items-stretch overflow-hidden">
                            <div
                              className={`flex items-center justify-between px-3 py-3.5 bg-white/[0.06] backdrop-blur-[12px] rounded-lg shadow-[0_2px_8px_rgba(0,0,0,0.12)] cursor-pointer select-none transition-[margin] duration-150 ease-linear min-w-0 flex-1 ${isActive ? 'mr-[90px]' : 'mr-0'}`}
