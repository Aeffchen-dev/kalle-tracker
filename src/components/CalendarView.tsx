              ...
              {filteredEvents.length === 0 && !isBirthdayToday ? (
                ...
                <div className="space-y-2 pb-20">
                  ...
                  {isBirthdayToday && birthdayAge > 0 && (
                    <div className="flex items-center justify-between p-3 bg-white/8 rounded-lg">
                  ...
                  {filteredEvents.map((event) => {
                    ...
                    return (
                      <div key={event.id} className="relative flex w-full items-stretch overflow-hidden">
                        <div
                          className={`flex items-center justify-between p-3 bg-white/8 rounded-lg cursor-pointer select-none transition-[margin] duration-150 ease-linear min-w-0 flex-1 ${isActive ? 'mr-[90px]' : 'mr-0'}`}
                        ...
                        >
                          <span className="text-[14px] text-white whitespace-nowrap flex items-center gap-2 overflow-hidden">
                            ...
                            <span className="truncate">
                              ...
                              {event.type === 'gewicht' && (
                                <>
                                  Gewicht: <span className={event.weight_value && isWeightOutOfBounds(Number(event.weight_value), new Date(event.time)) ? 'text-[#FF4444]' : 'text-[#5AD940]'}>{event.weight_value ? `${String(event.weight_value).replace('.', ',')} kg` : '-'}</span>
                                </>
                              )}
                              {event.type === 'phwert' && (
                                <>
