  ...
  useEffect(() => {
    ...
    const fetchWeather = (lat: number, lon: number) => {
      fetch(`https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&current=temperature_2m,weather_code&daily=temperature_2m_max,temperature_2m_min,weather_code,precipitation_probability_max,precipitation_sum&timezone=auto&forecast_days=14`)
        ...
        .then(data => {
          if (data.current) {
            setWeatherTemp(30); // TEMP TEST: was Math.round(data.current.temperature_2m)
            setWeatherEmoji(weatherCodeToEmoji(0)); // TEMP TEST: was data.current.weather_code
            setCurrentWeatherCode(0); // TEMP TEST: was data.current.weather_code
          }
          if (data.daily) {
