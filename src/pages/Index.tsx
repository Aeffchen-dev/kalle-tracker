            ...
            {forecast.filter(day => new Date(day.date + 'T00:00:00') >= new Date(format(new Date(), 'yyyy-MM-dd') + 'T00:00:00')).slice(0, 7).map((day) => {
              ...
              return (
                ...
                {isToday && weatherTemp !== null && weatherTemp <= 7 && (
                  <div className="rounded-lg py-1.5 px-3 bg-white/[0.06] flex items-center gap-3">
                    <img src={knittedHat} alt="cold" className="w-[20px] h-[20px] shrink-0 invert brightness-50" />
                    <span className="text-[#666666] text-[12px] leading-[16px]">Unter 7° braucht Kalle eine Jacke<br/>Unter 0° braucht er die dicke Winterjacke</span>
                  </div>
                )}
